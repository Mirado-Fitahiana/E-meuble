...
alter table mouvement alter column date_mouvement set default now();
...
SELECT 
  ...
  karama,
  date,
  CASE WHEN EXTRACT(YEAR FROM NOW()) - EXTRACT(YEAR FROM date) = 0 THEN 1
       ELSE EXTRACT(YEAR FROM NOW()) - EXTRACT(YEAR FROM date)
  END AS annee_travail
FROM 
  embauche_employer;
